...
#$ -S /bin/bash
#$ -N NK_0148
#$ -o /home/ha5653/backup/YachieLab/ikaken/NK_0148/out
#$ -e /home/ha5653/backup/YachieLab/ikaken/NK_0148/err
        ...
        for METHOD in rapidnjNJ raxmlMP fasttreeML; do 
            for mode in fractal original; do
                if [ $i -eq ${SGE_TASK_ID} ];then
                    ...
                    TREE_TOPOLOGY=${OUTPUT_DIR}/PRESUMEout/PRESUMEout.nwk
                    cp $TREE_TOPOLOGY PRESUMEout.nwk

                    N=$(cat ${OUTPUT_DIR}/PRESUMEout/PRESUMEout.fa.gz | gunzip | grep '>' | wc -l)

                    echo -n "${SGE_TASK_ID},${dir_id},${METHOD},${mode},${N},${m},${g},${L},${sigma}," > result.out
                    if   [ $mode = "original" ]; then
                        /usr/bin/time -f "%M,KB,%e,sec," timeout 86400 FRACTAL -i ${OUTPUT_DIR}/FRACTALin.fa.gz -m $METHOD -k 100          -t 10000000     -e 2>&1 1> /dev/null | tr -d "\n" >> result.out
                        python3 $PARTIAL PRESUMEout.nwk FRACTALout.nwk $TREEDISTR  >> result.out                    
                    elif [ $mode = "fractal" ]; then
                        /usr/bin/time -f "%M,KB,%e,sec," timeout 86400 FRACTAL -i ${OUTPUT_DIR}/FRACTALin.fa.gz -m $METHOD -k ${SUBSAMPLE} -t ${THRESHOLD} -e 2>&1 1> /dev/null | tr -d "\n" >> result.out
                        python3 $PARTIAL PRESUMEout.nwk FRACTALout.nwk $TREEDISTR  >> result.out                
                    fi
                    rm -r FRACTALout
                fi
